#!/usr/bin/env python3
"""
Today's Three Experiments (Sequential Runner)

Exp1: Direct 64x64 output (baseline loss weights)
Exp2: 1007 model (256x256 training) + evaluate at 64x64
Exp3: Direct 64x64 output + optimized loss weights

All experiments use timestamp: YYYYMMDD_HHMMSS
"""
...
class UNetPro64Direct(nn.Module):
    """Direct 64x64 output from d3 layer"""
...
class AdvancedLoss(nn.Module):
    def __init__(self, device, w_pcc=0.25, w_ssim=0.25, w_percep=0.35, w_edge=0.15):
        ...
        try:
            ...
            self.use_perceptual = (w_percep > 0)
        except:
    ...
    def forward(self, pred, target):
        ...
        if self.use_perceptual and self.w_percep > 0:
            ...
            # Normalize weights if perceptual is disabled
...
def evaluate(model, val_loader, device, eval_at_64=False):
    """
    Evaluate model
    eval_at_64: If True, downsample 256x256 output to 64x64 before evaluation
    """
    model.eval()
    total_pcc = 0.0
    total_ssim = 0.0
    ...
    with torch.no_grad():
        for x, gt in val_loader:
            ...
            if eval_at_64 and pred.shape[-1] == 256:
                pred = F.adaptive_avg_pool2d(pred, (64, 64))
                if gt.shape[-1] == 256:
                    gt = F.adaptive_avg_pool2d(gt, (64, 64))

            total_mse += F.mse_loss(pred, gt).item() * x.size(0)
            total_ssim += (1 - ssim_fn(pred, gt)).item() * x.size(0)

            for i in range(pred.shape[0]):
                pred_np = pred[i, 0].cpu().numpy().flatten()
                gt_np = gt[i, 0].cpu().numpy().flatten()
...
def run_single_experiment(exp_config, train_loader, val_loader, test_loader, device):
    ...
    # Create model
    if exp_config['model_type'] == 'unet256':
        model = UNetPro256(base=exp_config['base']).to(device)
    else:
        model = UNetPro64Direct(base=exp_config['base']).to(device)
    ...
    loss_fn = AdvancedLoss(
        device,
        w_pcc=exp_config['w_pcc'],
        w_ssim=exp_config['w_ssim'],
        w_percep=exp_config['w_percep'],
        w_edge=exp_config['w_edge']
    )

    print(f"Parameters: {sum(p.numel() for p in model.parameters()) / 1e6:.2f}M")
    print(f"Loss weights: PCC={exp_config['w_pcc']}, SSIM={exp_config['w_ssim']}, "
          f"Percep={exp_config['w_percep']}, Edge={exp_config['w_edge']}\n")
    ...
    for epoch in range(60):
        ...
        val_metrics = evaluate(model, val_loader, device, eval_at_64=exp_config['eval_at_64'])
    ...
    test_metrics = evaluate(model, test_loader, device, eval_at_64=exp_config['eval_at_64'])
...
def main():
    ...
    print("=" * 80)
    print("Today's Three Experiments (Sequential)")
    print("=" * 80)
    ...
    experiments = [
        {
            'name': 'exp1',
            'description': 'Direct 64x64 output (baseline loss weights)',
            'model_type': 'unet64',
            'base': 48,
            'dataset_type': 'dataset64',
            'eval_at_64': False,
            'w_pcc': 0.25,
            'w_ssim': 0.25,
            'w_percep': 0.35,
        ...
        {
            'name': 'exp2',
            'description': '1007 model (256x256) evaluated at 64x64',
            'model_type': 'unet256',
            'base': 48,
            'dataset_type': 'dataset256',
            'eval_at_64': True,  # KEY: Evaluate at 64x64
            'w_pcc': 0.25,
            'w_ssim': 0.25,
            'w_percep': 0.35,
            'w_edge': 0.15
        ...
        {
            'name': 'exp3',
            'description': 'Direct 64x64 output + optimized loss weights',
            'model_type': 'unet64',
            'base': 48,
            'dataset_type': 'dataset64',
            'eval_at_64': False,
    ...
    for exp_config in experiments:
        ...
        if exp_config['dataset_type'] == 'dataset64':
            train_dataset = Dataset64(speckle_file, pattern_file, train_indices)
            val_dataset = Dataset64(speckle_file, pattern_file, val_indices)
            test_dataset = Dataset64(speckle_file, pattern_file, test_indices)
        else:
        ...
        result = run_single_experiment(exp_config, train_loader, val_loader, test_loader, device)
    ...
    print(f"{'Experiment':<12} {'Test PCC':<12} {'Test SSIM':<12} {'Time (h)':<10}")
    print("-" * 80)

    for result in all_results:
        print(f"{result['experiment']:<12} "
    ...
    summary_path = os.path.join(BASE_DIR, f"summary_{summary['timestamp']}.json")
