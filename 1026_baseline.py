...
def ssim_loss(y_true, y_pred, window_size=11, sigma=1.5):
    ...
    mu1 = F.conv2d(y_true, window, padding=window_size // 2, groups=1)
    mu2 = F.conv2d(y_pred, window, padding=window_size // 2, groups=1)

    mu1_sq = mu1.pow(2)
    mu2_sq = mu2.pow(2)
    mu1_mu2 = mu1 * mu2

    sigma1_sq = F.conv2d(y_true * y_true, window, padding=window_size // 2, groups=1) - mu1_sq
    sigma2_sq = F.conv2d(y_pred * y_pred, window, padding=window_size // 2, groups=1) - mu2_sq
    sigma12 = F.conv2d(y_true * y_pred, window, padding=window_size // 2, groups=1) - mu1_mu2
